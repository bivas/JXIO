...
./autogen.sh && ./configure --silent --disable-raio-build --enable-silent-rules --enable-performance --enable-rdma=no && make -s && cp -f src/usr/.libs/libxio.so $BIN_FOLDER  && $STRIP_COMMAND $BIN_FOLDER/libxio.so
if [[ $? != 0 ]] ; then
